  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      ...
      >
        ...
        <span className="inline-flex h-7 w-7 cursor-pointer items-center justify-center rounded-full border border-slate-200 text-slate-600 transition hover:border-amber-400 hover:text-amber-400">
          {open ? "−" : "+"}
        </span>
      </button>
      ...
      {open && (
        <div className="mt-3 space-y-2">
          {section.options.map((option) => (
            <label key={option.value} className="flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 text-xs text-slate-700 transition hover:border-amber-300">
              <input
                type="checkbox"
